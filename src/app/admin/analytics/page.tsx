...
export default function AnalyticsTerminalPage() {
    ...
    // 1. Initialize High-Fidelity Registry Listeners
    useEffect(() => {
        ...
        if (groupBy === 'client') {
            ...
            return clientNames.map(name => {
                ...
                if (metric === 'assignments') value = clientJobs.length;
                    ...
                    // Estimate revenue/cost per client from jobs
                    value = clientJobs.reduce((acc, curr) => acc + (curr.pay || 0), 0);
                } else if (metric === 'hours') {
                    // Link daily logs to client via job
                    const clientJobIds = clientJobs.map(j => j.id);
                    // This is complex as daily logs don't directly link to job ID in current schema perfectly
                    // Fallback to 0 if not easily linked
                    value = 0; 
                }
        ...
        <div className="space-y-6">
            <header className="page-header flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div className="text-left">
                    ...
                    <h1 className="page-title">Intelligence Terminal</h1>
                    <p className="page-subtitle">Multi-dimensional operational audit and performance visualization.</p>
                </div>
                <div className="page-header-right items-center">
                    <Button variant="outline" size="sm" className="h-10 px-4 text-[10px] uppercase font-bold tracking-widest border-border-main" onClick={() => window.print()}>
                        <Download size={14} className="mr-2" /> Export Audit File
                    </Button>
                </div>
            </header>
                        ...
                        <CardContent className="p-5 space-y-6">
                                ...
                                <Popover>
                                    <PopoverTrigger asChild>
                                        ...
                                        >
                                            ...
                                            {dateRange && (
                                                <div className="absolute right-3" onClick={(e) => { e.stopPropagation(); setDateRange(undefined); }}>
                                                    <X size={14} className="text-text-muted hover:text-brand-red transition-colors" />
                                                </div>
                                            )}
                                        </Button>
                                    </PopoverTrigger>
                            ...
                            <div className="p-4 rounded-lg bg-bg-tertiary/50 border border-border-sub space-y-3">
                                ...
                                <p className="text-[9px] text-text-muted leading-relaxed uppercase font-medium">
                                    Terminal is currently aggregating data for <span className="text-brand-red">{metric}</span> grouped by <span className="text-brand-red">{groupBy}</span>.
                                </p>
                            </div>
                        </CardContent>
                        <CardFooter className="bg-bg-tertiary/30 border-t border-border-sub p-4">
                            <Button variant="ghost" className="w-full text-[9px] font-bold uppercase tracking-widest text-text-muted h-8" onClick={() => { setMetric('assignments'); setGroupBy('tech'); setDateRange({ from: new Date(new Date().getFullYear(), new Date().getMonth(), 1), to: new Date() }); }}>
                                <RefreshCw size={12} className="mr-2" /> Reset Terminal
                            </Button>
                        </CardFooter>
                    </Card>
                </div>
                            ...
                            {loading ? (
                                ...
                                <div className="text-center space-y-4 opacity-40">
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <p className="text-[10px] font-medium uppercase tracking-widest text-text-muted">Adjust the window or metric parameters in the control terminal.</p>
                                    </div>
                                </div>
                            )}
                        </CardContent>
