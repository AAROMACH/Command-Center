    ...
    return (
        <div className="flex flex-col gap-0.5 p-1 bg-transparent border border-border-sub rounded-md text-left w-fit min-w-[320px]">
             ...
             <div className="grid grid-cols-4 gap-1 pt-0.5 border-t border-border-sub/30 text-left">
                ...
                <div className="space-y-0 text-left">
                    <p className="text-[5px] font-black text-brand-red uppercase text-left">Aaromach Pay</p>
                                        ...
                                        {confirmedItems.length > 0 ? confirmedItems.map(item => {
                                            ...
                                            return (
                                                <div key={item.id} className={cn(
                                                    "p-2 rounded-lg border transition-all flex group gap-1 min-h-[3rem] items-center cursor-pointer",
                                ...
                                <ScrollArea className="h-full p-2 text-left">
                                    <div className="space-y-1 text-left">
                                        {(localLog?.items || []).filter(i => i.confirmationStatus === 'disputed').map(item => {
                                            ...
                                            return (
                                                <div key={item.id} className={cn(
                                                    "p-2 rounded-lg border transition-all flex flex-col group gap-1 min-h-[3rem] justify-center cursor-pointer",
                                                    isAudited ? "bg-bg-primary border-green-border/30" : "bg-bg-secondary border-brand-red/30 shadow-sm"
                                                )} onClick={() => wo && handleOpenJobDetail(wo)}>
                                                    <div className="flex items-center gap-4 flex-1 text-left">
                                                        <div className="shrink-0 flex items-center gap-2 text-left" onClick={e => e.stopPropagation()}>
                                                            <Button 
                                                                variant="outline" 
                                                                size="sm" 
                                                                className={cn(
