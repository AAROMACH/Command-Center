...
import { Upload, FileText, Image as ImageIcon, Download, Trash2, FolderOpen, Milestone, Camera, Paperclip, Plus } from 'lucide-react';
...
const MilestoneDocuments = ({ phase, documents }: { phase: Phase, documents: ProjectDocument[] }) => {
    ...
    const findPhotoForTask = (taskId: string) => {
        return documents.find(doc => doc.taskId === taskId && doc.type === 'img');
    };
    ...
    return (
        <div className="field-group !p-0">
             ...
             <div className="p-4 space-y-4">
                ...
                <div>
                    <label className="field-label">Required Photos</label>
                    {requiredPhotoTasks.length > 0 ? (
                        <div className="space-y-2">
                            {requiredPhotoTasks.map(task => {
                                const photo = findPhotoForTask(task.id);
                                return (
                                    <div key={task.id} className="required-photo-row">
                                        ...
                                        {photo ? (
                                            <div className="flex items-center gap-2">
                                                <div className="relative h-9 w-9 rounded-md border border-border-default p-0.5">
                                                    {photo.url && <Image src={photo.url} alt={photo.name} width={32} height={32} className="rounded-sm object-cover" />}
                                                </div>
                                                <div className="text-xs">
                                                    <div className="font-semibold text-text-primary">{photo.name}</div>
                                                    <div className="text-text-muted">{photo.size}</div>
                                                </div>
                                                <Button variant="outline" size="icon-sm" className="doc-btn"><Trash2 size={13}/></Button>
                                            </div>
                                        ) : (
                ...
                <div>
                    <label className="field-label">Other Phase Documents</label>
                    <div className="space-y-2">
                        ...
                        <Button variant="dashed" size="sm" className="w-full !h-9 !text-xs">
                            <Paperclip size={14} className="mr-2" /> Add a document to this phase
                        </Button>
                    </div>
                </div>
