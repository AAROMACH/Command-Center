        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="email" type="email" disabled={isLoading} placeholder="admin@aaromach.com" className="bg-bg-primary border-border-sub h-11 text-xs" {...register("email")} />
          ...
          <CardFooter className="flex flex-col gap-4">
            <Button type="submit" disabled={isLoading} className="w-full h-12 bg-brand-red hover:bg-brand-red-hover text-white font-bold uppercase tracking-widest text-xs">
              {isLoading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Synchronizing...</> : "Login"}
            </Button>
            <p className="text-[9px] text-text-muted uppercase font-bold tracking-widest text-center px-4">Authorized personnel only. All access events are logged in the system registry.</p>
          </CardFooter>
        </form>
      </Card>
    </div>
